...
/*   Updated: 2025/02/03 12:18:25 by fruan-ba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv, char **envp)
{
	...
	root = NULL;
	init_utils(&data);
	root = create_token("find", CMD);
	if (!root)
		return (1);
	add_token(&root, "/home/user", ARG);
	add_token(&root, "-type", ARG);
	add_token(&root, "f", ARG);
	add_token(&root, "-name", ARG);
	add_token(&root, "*.txt", ARG);
	add_token(&root, "|", PIPE);
	add_token(&root, "xargs", CMD);
	add_token(&root, "grep", CMD);
	add_token(&root, "-H", ARG);
	add_token(&root, "minishell", ARG);
	add_token(&root, "|", PIPE);
	add_token(&root, "sort", CMD);
	add_token(&root, "-u", ARG);
	add_token(&root, ">", REDIRECT_OUT);
	add_token(&root, "/home/user/minishell_files.txt", FD);
	add_token(&root, "&&", OPERATOR_AND);
	add_token(&root, "echo", CMD);
	add_token(&root, "Busca concluída", ARG);
	show_tokens(root);
