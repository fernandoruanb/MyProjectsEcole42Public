...
/*   Updated: 2025/02/02 13:06:24 by fruan-ba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv, char **envp)
{
	...
	if (!root)
		return (1);
	add_token(&root, "/", ARG);
	add_token(&root, "-type f", ARG);
	add_token(&root, "-name *.txt", ARG);
	add_token(&root, "-or -name .log", ARG);
	add_token(&root, "|", PIPE);
	add_token(&root, "xargs", CMD);
	add_token(&root, "grep", CMD);
	add_token(&root, "-i", ARG);
	add_token(&root, "erro crítico", ARG);
	add_token(&root, "|", PIPE);
