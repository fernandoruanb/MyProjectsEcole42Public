...
/*   Updated: 2025/02/02 13:20:07 by fruan-ba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv, char **envp)
{
	...
	if (!root)
		return (1);
	add_token(&root, "arquivo1.txt", ARG);
	add_token(&root, "arquivo2.txt", ARG);
	add_token(&root, "arquivo3.txt", ARG);
	add_token(&root, "arquivo4.txt", ARG);
	add_token(&root, "arquivo5.txt", ARG);
	add_token(&root, "arquivo6.txt", ARG);
	add_token(&root, "arquivo7.txt", ARG);
	add_token(&root, "/tmp/backup/", ARG);
	add_token(&root, "&&", OPERATOR_AND);
	add_token(&root, "echo", CMD);
	add_token(&root, "Cópia concluída", ARG);
	show_tokens(root);
