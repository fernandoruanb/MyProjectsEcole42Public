...
/*   Updated: 2025/02/03 08:52:05 by fruan-ba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv, char **envp)
{
	...
	if (argc != 2)
		return (1);
	init_utils(&data);
	root = create_token("make", CMD);
	if (!root)
		return (1);
	add_token(&root, "&&", OPERATOR_AND);
	add_token(&root, "make", CMD);
	add_token(&root, "test", ARG);
	add_token(&root, "||", OPERATOR_OR);
	add_token(&root, "echo", CMD);
	add_token(&root, "fail", ARG);
	show_tokens(root);
