...
/*   Updated: 2025/02/03 12:49:57 by fruan-ba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv, char **envp)
{
	...
	root = NULL;
	init_utils(&data);
	root = create_token("python", CMD);
	if (!root)
		return (1);
	add_token(&root, "script.py", ARG);
	add_token(&root, ">", REDIRECT_OUT);
	add_token(&root, "output.log", FD);
	add_token(&root, "2", FD);
	add_token(&root, ">", REDIRECT_OUT);
	add_token(&root, "error.log", FD);
	show_tokens(root);
