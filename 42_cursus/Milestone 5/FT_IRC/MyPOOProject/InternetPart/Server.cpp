...
/*   Updated: 2025/07/08 17:27:28 by fruan-ba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
struct pollfd	(&Server::getPollFds(void) const)[1024]
{
	return (fds);
}
...
void	Server::init(int port, std::string password)
{
	...
	try {
		...
		signal(SIGTERM, this->handleSignal);
...
void	Server::addNewClient(int clientFD)
{
	...
	fcntl(fds[index].fd, F_SETFL, O_NONBLOCK);
	this->numClient++;
}
...
void	Server::privmsg(int index, std::string message)
{
	...
	if (message.empty() || fds[index].fd == -1)
		return ;
...
void	Server::manageBuffers(int index)
{
	...
	this->sendBuffer[index].clear();
	this->sendBuffer[index] = this->sendBuffer[this->numClient - 1];
	this->sendBuffer[this->numClient - 1].clear();
}
