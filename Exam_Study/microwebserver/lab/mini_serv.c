...
static void	broadcast(t_server *myServer, int owner, fd_set *active_fds, int flag)
{
	int	fd;
	char	msg[4098];
	int	index = 0;
	...
	if (flag == 0)
	{
		...
		while (fd <= myServer->fd_max)
		{
			...
			if (FD_ISSET(fd, active_fds))
			{
				...
				sprintf(msg, "client %d: ", myServer->clients[owner]);
				while (*ptr)
				{
					char	tmp;
					if (*ptr != '\0')
...
{
	size_t	index;
	size_t	length = sizeof(myServer->buffer);
			...
			if (FD_ISSET(fd, &read_fds))
			{
				if (fd == myServer->serverFD)
				{
					...
					if (clientFD >= 0)
					{
						...
						broadcast(myServer, clientFD, &active_fds, 2);
						myServer->next_id++;
					}
					else
					{
				...
				else
				{
					...
					if (bytes <= 0)
					{
						broadcast(myServer, fd, &active_fds, 1);
					...
					else
					{
						...
						broadcast(myServer, fd, &active_fds, 0);
					}
				}
			}
			++fd;
